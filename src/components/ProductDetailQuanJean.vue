...
import ProductQuanJean from "@/data/itemsQuanJean";
...
export default {
  ...
  mounted() {
    ...
    this.product = ProductQuanJean.find((x) => x.id == id);
  },
  methods: {
    addToCart(product) {
      // Kiểm tra nếu sản phẩm với cùng kích thước đã có trong giỏ hàng
      const infoProduct = this.cart.find((item) => item.id == product.id && item.size == this.selectedSize && item.category == "QuanJean");
      if (infoProduct) {
