...
import ProductQuanShort from "@/data/itemsQuanShort";
export default {
  data() {
    return {
      product: {},
  ...
  mounted() {
    ...
    this.product = ProductQuanShort.find((x) => x.id == id);
  },
  methods: {
    addToCart(product) {
      // Kiểm tra nếu sản phẩm với cùng kích thước đã có trong giỏ hàng
      const infoProduct = this.cart.find(
        (item) => item.id == product.id && item.size == this.selectedSize && item.category == "QuanShort");
      if (infoProduct) {
